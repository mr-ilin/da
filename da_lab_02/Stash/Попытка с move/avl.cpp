...
TAvlNode* TAvlTree::Balance(TAvlNode* node) {
    ...
    RecountHeight(node);
    uint64_t balance = GetBalance(node);
...
// Поиск узла по ключу
TAvlNode* TAvlTree::Find(TString key) const {
...
TAvlNode* TAvlTree::InsertInNode(TAvlNode* node, TString key, uint64_t value) {
    if (!node) {
        std::cout << "OK\n";
    ...
    if (key < node->key) {
        ...
        node->left = InsertInNode(node->left, std::move(key), value);
    } else if (key > node->key) {
        //node->right = InsertInNode(node->right, key, value);
        node->right = InsertInNode(node->right, std::move(key), value);
    } else {
        std::cout << "Exist\n";
    }
...
void TAvlTree::Insert(TString key, uint64_t value) {
    //root = InsertInNode(root, key, value);
    root = InsertInNode(root, std::move(key), value);
}
...
TAvlNode* TAvlTree::SubRemove(TAvlNode* node, TString key) {
    if (!node) {
        std::cout << "NoSuchWord\n";
        return nullptr;
    }
...
// Обертка над Remove
void TAvlTree::Remove(TString key) {
    root = SubRemove(root, std::move(key));
}
...
void TAvlTree::Save(TString& path) {
...
void TAvlTree::Load(TString& path) {
...
int TAvlTree::CountTreeHeight(TAvlNode* node) const {
    if (!node) {
        return 0;
    }

    return std::max(CountTreeHeight(node->left), CountTreeHeight(node->right)) + 1;
}

void TAvlTree::DfsPrint(const TAvlNode* node, int depth) const{
    if (!node) {
        return;
    }
